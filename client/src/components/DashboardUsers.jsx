...
const [isUserRoleUpdated, setIsUserRoleUpdated] = useState(false)
  ...
  return(
    ...
    <p className='text-textColor text-base w-275 min-w-[160px] text-center'>{data.email}</p>
    <p className='text-textColor text-base w-275 min-w-[160px] text-center'>{data.email_verified?"True":"False"}</p>
    <p className='text-textColor text-base w-275 min-w-[160px] text-center'>{createdAt}</p>
    {/* <p className='text-textColor text-base w-275 min-w-[160px] text-center'>{data.role}</p> */}
    <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative'>
    <p className='text-textColor text-base w-175 min-w-[10px] text-center'>{data.role}</p>
    {
      data._id !== user?.user._id && (
...
<p className='text-textColor text-sm font-semibold'>Are you Sure you want to make this user&nbsp;
...
<div className='flex items-center gap-4'>
        ...
        <motion.button whileTap={{scale:0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-green-200 text-black hover:shadow-md' onClick={()=>setIsUserRoleUpdated(false)}>
          No
